...
def evalrank(model_path, data_path=None, split='dev', fold5=False):
    ...
    if not fold5:
        ...
        start = time.time()
        sims = shard_xattn(model, img_embs, cap_embs, cap_lens, opt, shard_size=128)
        end = time.time()
...
def shard_xattn(model, images, captions, caplens, opt, shard_size=128):
    """
    Computer pairwise image-text distance with locality sharding
    """
    ...
    for i in range(n_im_shard):
        ...
        for j in range(n_cap_shard):
            ...
            im = Variable(torch.from_numpy(images[im_start:im_end]), volatile=True).cuda().float()
            s = Variable(torch.from_numpy(captions[cap_start:cap_end]), volatile=True).cuda().float()
